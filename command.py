...
def _status_text(engine) -> str:
    ...
    lines = [
        ...
        f"post_compaction_target_tokens: {status.get('post_compaction_target_tokens') or '(none)'}",
        f"emergency_pressure_ratio: {status.get('emergency_pressure_ratio', engine._config.emergency_pressure_ratio)}",
        f"emergency_threshold_tokens: {status.get('emergency_threshold_tokens') or '(disabled)'}",
