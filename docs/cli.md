...
`supported_schema_version` (currently 13). The CLI can inspect older databases
without migrating them, reads schema v13 (including protected-session, rollover-head,
durable session-end receipt, and exact node-provenance proof counts), and exits with database failure code
